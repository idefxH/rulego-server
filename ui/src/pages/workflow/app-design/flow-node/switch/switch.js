...
import VueNode from '@src/pages/workflow/app-design/flow-node/switch/switch.vue';
import {
  NODE_TITLE_HEIGHT,
  NODE_DEFAULT_WIDTH,
  NODE_DEFAULT_HEIGHT,
...
class VueHtmlNodeModel extends HtmlNodeModel {
  ...
  getDefaultAnchor() {
    ...
    const defaultAnchorIndex = anchors.findIndex(
      (item) => item.id === 'Default',
    );
    if (defaultAnchorIndex !== -1) {
      const defaultAnchor = cloneDeep(anchors[defaultAnchorIndex]);
      anchors.splice(defaultAnchorIndex, 1);
